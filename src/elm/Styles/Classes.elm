module Helpers.Styles exposing (tachs)


tachs =
    { main = "tc w-100 h-100vh flex flex-column items-center justify-center bg-dark-blue"
    , svgcontainer = "tc dib w-80"
    , svg = "svg-p overflow-visible"
    , date = "sans-serif washed-blue tracked-mega fw3"
    , svgtext = "f60 sans-serif"
    , svgtextcol = "rgb(130,130,130)"
    , nav = "flex justify-center"
    , navbutton = "bn pointer bg-transparent outline-0 washed-blue pa4"
    , navtext = "sans-serif washed-blue tracked-mega fw3"
    , error = "sans-serif"
    }
